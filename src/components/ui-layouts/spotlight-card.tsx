...
export function SpotLightItem({ children, className }: SpotlightItemProps) {
  ...
  return (
    ...
    >
      {isHovered && (
        <div
          className="pointer-events-none absolute opacity-0 z-50 rounded-md w-full h-full group-hover:opacity-100  transition duration-300 "
          style={{
            background: `
            radial-gradient(
      ...
      {HoverFocusSpotlight && (
        <div
          className="absolute opacity-0 group-hover:opacity-100 z-10 inset-0 bg-fixed rounded-md"
          style={{
      ...
      {ProximitySpotlight && (
        <div
          className="absolute inset-0 z-0  bg-fixed rounded-md"
          style={{
